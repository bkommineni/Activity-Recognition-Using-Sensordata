...
public class ESSensorManager implements ESSensorManagerInterface, SensorDataListener
{
	...
	public static ESSensorManager getSensorManagerWithCustomConfig(final Context context, SensorConfig sensorConfig) throws ESException
	{
		if (context == null)
		{
			throw new ESException(ESException.INVALID_PARAMETER, " Invalid parameter, context object passed is null");
		...
		if (sensorManager == null)
		{
			synchronized (lock)
			{
				if (sensorManager == null)
				{
					sensorManager = new ESSensorManager(context,sensorConfig);
				}
			}
		}
		return sensorManager;
	...
	private ESSensorManager(final Context appContext,SensorConfig sensorConfig)
	{
		...
		ArrayList<SensorInterface> sensors = SensorUtils.getAllSensorsWithCustomConfig(appContext,sensorConfig);
	...
	public void startSensingData(int sensorId) throws ESException
	{
		AbstractSensorTask sensorTask = getSensorTask(sensorId);
		if (!SensorUtils.isPullSensor(sensorTask.getSensorType()))
		{
		...
		{
			((PullSensorTask) sensorTask).startSensingData();
		}
	}

	public SensorData stopSensingData(int sensorId) throws ESException
	{
		SensorData sensorData = null;
		AbstractSensorTask sensorTask = getSensorTask(sensorId);
		if (!SensorUtils.isPullSensor(sensorTask.getSensorType()))
		...
		{
			sensorData = ((PullSensorTask) sensorTask).stopSensingData();
		}
		return sensorData;
	}
