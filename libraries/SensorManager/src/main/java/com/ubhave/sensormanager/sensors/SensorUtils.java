...
public class SensorUtils
{
	...
	public static ArrayList<SensorInterface> getAllSensorsWithCustomConfig(final Context applicationContext,SensorConfig sensorConfig)
	{
		ArrayList<SensorInterface> sensors = new ArrayList<>();
		for (SensorEnum s : SensorEnum.values())
		{
			try
			{
				SensorInterface sensor = getSensorWithCustomConfig(s.getType(), applicationContext,sensorConfig);
				sensors.add(sensor);
			}
			catch (ESException e)
			{
	...
	private static SensorInterface getSensorWithCustomConfig(int id, Context context, SensorConfig sensorConfig) throws ESException
	{
		switch (id)
		{
			case SENSOR_TYPE_ACCELEROMETER:
				return AccelerometerSensor.getSensorWithCustomConfig(context,sensorConfig);
			case SENSOR_TYPE_BATTERY:
				return BatterySensor.getSensor(context);
			case SENSOR_TYPE_BLUETOOTH:
				return BluetoothSensor.getSensor(context);
